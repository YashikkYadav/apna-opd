<template>
  <v-container fluid>
    ...
    <v-data-table
      ...
      item-value="id"
      show-select
      class="tablee template-table grey-head queue"
    >
  ...
  <common-model
    :commonModel="isCompletedModalOpen"
    @close-dialog="isCompletedModalOpen = false"
    @actions="completeAppointment"
    title="Mark Completed"
    description="Are you sure you want to complete this appointment?"
  />

  <common-model
    :commonModel="isNoShowModalOpen"
    @close-dialog="isNoShowModalOpen = false"
    @actions="noShowAppointment"
    title="Mark No Show"
...
export default {
  ...
  methods: {
    async fetchAppointments() {
      ...
      const res = await useProfileStore().getDoctors();
      console.log(res);

      if (res) {
        this.doctors = res.doctorData.map((doctor) => ({
          id: doctor._id,
    ...
    async completeAppointment() {
      ...
      const res = await useAppointmentStore().UpdateAppointmentStatusApiCall(
        requestData
      );
      ...
      if (res) {
        ...
        useUiStore().openNotificationMessage(
          "Appointment Mark As Completed Succesfully!"
        );
      }
    },
    async noShowAppointment() {
      ...
      if (res) {
        ...
        this.fetchAppointments();
        useUiStore().openNotificationMessage(
          "Appointment Mark As No Show Succesfully!"
        );
      }
    },
    markCompleted() {
