    ...
    async fetchProfileData() {
      ...
      if (hs || profile) {
        this.form = {
          ...
          // Availability
          delay: profile?.availabilityAfter || "",
          from: profile?.unavailabilityDate?.from
            ? profile.unavailabilityDate.from.split("T")[0]
            : "",
          to: profile?.unavailabilityDate?.to
            ? profile.unavailabilityDate.to.split("T")[0]
            : "",

          // Arrays
          testimonials: profile?.testimonials || [],
          conditionsTreated: profile?.conditionsTreated || [],
          proceduresOffered: profile?.proceduresOffered || [],
          membershipAwards: profile?.membershipAwards || [],
          faqs: profile?.faqs || [],
