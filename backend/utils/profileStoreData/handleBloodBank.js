...
exports.handlePhysiotherapist = async (req, healthServeId) => {
  try {
    const files = req.files || [];
    const existing = await physiotherapistsProfileModel.findOne({
      healthServeId,
    });
...
exports.handleBloodBank = async (req, healthServeId) => {
  try {
    ...
    const finalData = {
      ...
      nearbyBloodBanks: JSON.parse(req.body.nearbyBloodBanks).map(
        (i) => i.name
      ),
...
exports.handleGym = async (req, healthServeId) => {
  try {
    ...
    const finalData = {
      ...
      trainers:
        typeof req.body.trainers === "string"
          ? JSON.parse(req.body.trainers)
          : req.body.trainers,
...
exports.getHandleGym = async (healthServeId) => {
  const doc = await gymProfileModel
...
exports.gethandleBloodBank = async (healthServeId) => {
  const doc = await bloodBankProfileModel
    .findOne({ healthServeId })
    .populate("healthServeId");
  return doc;
};
...
exports.gethandlePhysiotherapist = async (healthServeId) => {
  const doc = await physiotherapistsProfileModel
    .findOne({ healthServeId })
    .populate("healthServeId");
...
exports.gethandleMedicalStore = async (healthServeId) => {
  const doc = await pharmacyProfileModel
    .findOne({ healthServeId })
    .populate("healthServeId");
  return doc;
};
...
exports.gethandleLaboratory = async (healthServeId) => {
  const doc = await healthlabProfileModel
    .findOne({ healthServeId })
    .populate("healthServeId");
  return doc;
};
...
exports.gethandleIvf = async (healthServeId) => {
  const doc = await ivfClinicModel
    .findOne({ healthServeId })
    .populate("healthServeId");
  return doc;
};
