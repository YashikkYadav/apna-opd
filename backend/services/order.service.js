...
const createOrder = async (healthServeId, data) => {
  try {
    ...
    // Find pharmacy profile using healthServeId (not _id directly)
    const pharmacyData = await PharmacyProfile.findOne({ healthServeId });
    if (!pharmacyData) {
    ...
    // Update stock directly (no validation)
    for (let orderItem of items) {
      const medicine = pharmacyData.medicines.id(orderItem.medicineId);
      console.log("Medicine Found:", medicine);
      if (medicine) {
        medicine.stock = medicine.stock - orderItem.quantity;
      }
    }

    // Save updated pharmacy data
