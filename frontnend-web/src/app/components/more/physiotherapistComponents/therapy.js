...
const TherapyPackages = ({ data, healthProfile }) => {
    ...
    return (
      ...
      >
        {/* Title */}
        <div className="mb-8">
          <h2 className="text-lg md:text-4xl font-extrabold text-blue-700 flex items-center gap-3">
            <Package className="text-3xl text-blue-700 w-10 h-10"  />
            <p className="mt-4 md:mt-10">
              Therapy Packages for{" "}
              <span className="text-blue-800">{data?.name ?? "Doctor"}</span>
            </p>
          </h2>
        </div>
            ...
            >
              <div>
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-lg text-black">{pkg.name}</h3>
                  {pkg?.discount && (
