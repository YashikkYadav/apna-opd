...
export default function HospitalTestimonialsCard({ hospitalData }) {
  const testimonials = hospitalData?.testimonials || [];
  return (
    <motion.div
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {testimonials.map((t, idx) => {
          const stars = Math.max(0, Math.min(5, Number(t.stars) || 0));
          return (
            <motion.div
              key={idx}
