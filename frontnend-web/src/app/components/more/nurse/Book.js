...
export default function FreeTrialModal({ isOpen, onClose,healthId }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors("Something went wrong. Provide Valid details");
    }
  };
        ...
        {!submitted ? (
          <>
            ...
            <textarea
              ...
              rows={3}
              className={`w-full mb-1 px-4 py-2 border rounded-lg resize-none focus:outline-none focus:ring-2 text-gray-800 ${
                errors.message
                  ? "border-red-400 ring-red-300"
                  : "border-blue-300 focus:ring-blue-500"
              }`}
            />
            {errors.message && (
              <div className="text-red-600 text-xs mb-2">{errors.message}</div>
            )}
